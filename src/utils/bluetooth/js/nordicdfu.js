...
import {
  transmitNordicDfuControlData,
  transmitNordicDfuPacketData,
} from "./bluetooth.js";
...
async function transferFile(file, type) {
  ...
  for (let chk = 0; chk < chunks; chk++) {
    ...
    const chunkCrc = crc32(
      new Uint8Array(file).slice(0, fileOffset + chunkSize)
    );

    console.log(
      "chunk " + chk + ", fileOffset: ",
      +fileOffset + ", chunkSize: " + chunkSize + ", chunkCrc",
      +chunkCrc
    );
    ...
    console.log(
      "returnedOffset: " + returnedOffset + ", returnedCrc: " + returnedCrc
    );

    if (returnedCrc != chunkCrc) {
      return Promise.reject(
        "CRC mismatch after sending this chunk. Expected: " + chunkCrc
      );
    }
